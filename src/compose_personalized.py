"""
Персонализированный композер push-уведомлений по принципу "персонализированная лаконичность"
"""
import pandas as pd
import numpy as np
import yaml
...
class PushComposer:
    """Персонализированный композер push-уведомлений"""
    ...
    def __init__(self, config_path: str = "conf/weights.yaml", 
        ...
        if api_key:
            ...
            if not api_key:
                logger.warning("⚠️ OpenAI API ключ не найден. Используется персонализированный fallback режим.")
                self.client = None
                ...
                try:
                    ...
                    logger.warning(f"Ошибка инициализации OpenAI: {e}. Используется fallback.")
                    self.client = None
                    self.llm_available = False
...
"""
    ...
    def generate_push(self, client_data: Dict[str, Any], product: str, 
                     details: Dict[str, Any]) -> str:
        """Генерация персонализированного push"""
        ...
        if not self.llm_available or not self.client:
            return self._generate_personalized_fallback(client_data, product)
        ...
        try:
            ...
            logger.warning(f"Ошибка LLM: {str(e)}. Используем персонализированный fallback.")
            return self._generate_personalized_fallback(client_data, product)
    ...
    def _generate_personalized_fallback(self, client_data: Dict[str, Any], product: str) -> str:
        """Персонализированные fallback с конкретными цифрами"""
        ...
        if product == "Карта для путешествий":
            ...
            return f"{name}, в августе вы сделали {trip_count} поездок на {self._format_currency(total_travel)}. Тревел-карта вернула бы {self._format_currency(cashback)} кешбэком. Оформить карту."
            ...
            return f"{name}, вы часто в ресторанах и бутиках ({self._format_currency(monthly_premium)}/мес). Премиальная карта вернёт {tier}% с этих трат. Подключить карту."
            
        elif product == "Кредитная карта":
            total_spend = client_data.get('total_spend', 0)
            potential_cashback = min(total_spend * 0.10, 90000)
            top_cat1 = client_data.get('top_category_1', 'продукты питания')
            top_cat2 = client_data.get('top_category_2', 'кафе и рестораны')
            
            return f"{name}, ваши топ-категории — {top_cat1.lower()}, {top_cat2.lower()}. Кредитка вернула бы {self._format_currency(potential_cashback)} кешбэком. Оформить карту."
            
        elif product == "Обмен валют":
            fx_volume = client_data.get('fx_volume', 0)
            savings = fx_volume * 0.01
            
            return f"{name}, вы часто меняете валюту ({self._format_currency(fx_volume)}). В приложении выгодный курс 24/7 без комиссии. Настроить обмен."
            
        elif product == "Депозит Сберегательный":
            free_funds = client_data.get('free_funds', 0)
            monthly_income = free_funds * 0.165 / 12
            
            return f"{name}, у вас {self._format_currency(free_funds)} лежат без дела. Сберегательный вклад под 16,5% даст {self._format_currency(monthly_income)}/мес. Открыть вклад."
            
        elif product == "Депозит Накопительный":
            free_funds = client_data.get('free_funds', 0)
            monthly_income = free_funds * 0.155 / 12
            
            return f"{name}, у вас {self._format_currency(free_funds)} могут приносить доход. Накопительный вклад под 15,5% даст {self._format_currency(monthly_income)}/мес. Открыть вклад."
            
        elif product == "Депозит Мультивалютный":
            free_funds = client_data.get('free_funds', 0)
            fx_volume = client_data.get('fx_volume', 0)
            
            return f"{name}, у вас валютные операции на {self._format_currency(fx_volume)} и {self._format_currency(free_funds)} свободных. Мультивалютный депозит под 14,5%. Открыть."
            ...
            return f"{name}, вам {age} лет и {self._format_currency(free_funds)} могут работать. Инвестиции от 6₸ дадут ~{self._format_currency(potential_return)}/мес. Открыть счёт."
            
        elif product == "Золотые слитки":
            balance = client_data.get('avg_monthly_balance_KZT', 0)
            gold_allocation = balance * 0.15
            
            return f"{name}, с балансом {self._format_currency(balance)} можно диверсифицировать в золото. Рекомендуем {self._format_currency(gold_allocation)}. Узнать подробнее."
            ...
            return f"{name}, расходы превышают доходы на {self._format_currency(shortage)}. Кредит до {self._format_currency(limit)} от 12% без залога. Узнать лимит."
        
        # Универсальный fallback
        return f"{name}, выгодное предложение по {product}. Узнать подробности в приложении."
    
    def compose_push_for_clients(self, features: pd.DataFrame, details: pd.DataFrame, 
                                ranked_products: pd.DataFrame) -> pd.DataFrame:
        """Генерация персонализированных push для всех клиентов"""
        mode = "LLM" if self.llm_available else "Персонализированный Fallback"
        logger.info(f"Начинаем генерацию push-уведомлений ({mode} режим)")
        ...
        for _, row in best_products.iterrows():
            ...
            # Генерируем персонализированный push
            push_text = self.generate_push(client_data, product, product_details)
            
            # Убеждаемся что длина правильная
            if len(push_text) < 180:
                push_text += " Оформление займёт пару минут."
        ...
        mode = "LLM" if self.llm_available else "Персонализированный Fallback"
        logger.info(f"✓ Сгенерировано {len(recommendations)} push-уведомлений ({mode} режим)")
        
        return result_df


def test_personalized_composer():
    """Тест персонализированного композера"""
    test_client = {
        'client_code': 1,
        'name': 'Айгерим',
        'age': 29,
